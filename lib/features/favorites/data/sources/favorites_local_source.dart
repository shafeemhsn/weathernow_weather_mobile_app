...
class FavoritesLocalSource {
  FavoritesLocalSource([SharedPreferences? prefs])
  ...
  Future<List<FavoriteCityModel>> fetch({String query = ''}) async {
    ...
    final filtered = query.isEmpty
        ...
        : stored
              ...
              .toList();
    return filtered.map((name) => FavoriteCityModel(name: name)).toList();
  }

  Future<void> add(FavoriteCityModel city) async {
    final prefs = await _prefsFuture;
    final stored = prefs.getStringList(_storageKey) ?? <String>[];
  ...
  Future<void> remove(FavoriteCityModel city) async {
    final prefs = await _prefsFuture;
    final stored = prefs.getStringList(_storageKey) ?? <String>[];
